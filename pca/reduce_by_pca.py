...
The two methods themselves provide score methods.
...
def best_components_PCA_FA(X):
    ...
    # #############################################################################
    # Fit the models
    n_components = np.arange(5, 201, 10)  # options for n_components
...
def plot_cev(X, title):
    ...
    ticks = [x for x in range(X.shape[1] + 1) if x % 16 == 0]  #np.linspace(0,X.shape[1],5)
    plt.xticks(ticks)
    plt.grid()
    plt.show()
...
if __name__ == '__main__':
    ...
    # X = load_fuel_truck_data().iloc[:, 1:num_features_freq+1]
    # plot_cev(X, 'cumulative explained variance ratio of freq features')
    # X = load_fuel_truck_data().iloc[:, num_features_freq+1:]
